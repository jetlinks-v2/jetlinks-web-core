...
- 选择后持久化仍只保存 `{ capabilityId, capabilityVersion }`；label、description、metadata 和 contract 都是可重新解析的展示数据，不进入稳定引用。
...
Operation 的 `contract` 对应为 `action/configSchema/paramsSchema/resultSchema/policy`。用户确认选择后只持久化 `{ capabilityId: item.value, capabilityVersion: item.version }`，其余字段刷新目录时重新解析。
...
`query()`、`connect()`、`preview()` 和 `DataCapabilityClient.query()` 共用 `mapping.ts` 中的同一个执行器。consumer 可传 `targetSchema`，mapping 后的最终结果不匹配时统一抛出 `capability.validation_failed`；preview 与真实运行不再出现两套转换结果。`OutputMapping.version`、data/operation binding version 和 plan version 都会先校验，未知版本不会进入 Provider。

当前明确不执行 `format`、expression 和 coerce：即使不受类型约束的外部 JSON 传入这些规则，也分别抛出结构化 unsupported 错误，不再静默返回 `undefined`。后续字段自动匹配、受控类型转换、日期格式和诊断建议仍应使用新 mapping version 扩展；不得把任意 JavaScript 纳入执行链。自动匹配只能生成建议，歧义、信息丢失和不安全转换必须要求用户确认。
...
当前执行会校验 data binding、operation binding、mapping、plan 和 capability version。未知容器版本分别明确拒绝，capability 引用与 definition 不一致时抛出 `capability.version_mismatch`。`CapabilityMigration` 尚未进入 Runtime，当前不能宣称自动迁移。
...
| 版本门禁 | 已实现 | 拒绝未知 data/operation binding、mapping、plan 版本及 capability mismatch | version guards、core/output tests |
...
4. 后续使用新 mapping version 增加 source/target schema 字段建议、受控类型转换和 format，明确 null、缺失字段、转换失败和 fallback 行为；禁止使用 locale 或浏览器隐式转换产生不稳定结果。
