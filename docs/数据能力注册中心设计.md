...
带 `scopeSelection` 的分析 producer 通过唯一 `analyticalCapability.argumentBindings` 声明范围，而不是向 Provider 或 HTTP 参数下沉协议状态。共享 scope helper 固定发布无默认值的必填 `scopeMode`：`project` 分支禁止 `areaId`/`pointId`，`explicit` 分支恰好要求其中一个；交叉的时间等其它输入 alternatives 保持其自身约束。binding 同时携带 `modeArgument`，重复或与 limit、filter、dimension、temporal、consumer 参数冲突的定义在会话 wire 前失败关闭。验证由 `test:client-tools` 覆盖 closed union、round-trip、冲突与 malformed schema，并由 `test:client-tool-types` 覆盖 authoring 类型边界。
