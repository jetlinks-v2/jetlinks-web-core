...
- `cloud.jetlinks.ui` 集成 PR 继续承载可视化 legacy binding adapter、子模块指针和宿主侧集成验证。core 侧测试覆盖 catalog、Provider 无参 single-flight 加载、Provider 并发注销 ghost 防护、手动 Provider 加载失败隔离、Provider 注册/卸载、直接 register 注销联动 Runtime、直接 register override 注销后恢复上一层定义、同 capabilityId 不同 Provider 代次精确清理、持久化 version mismatch、参数绑定、query 取消 reject 与 teardown、connect 异步 create 期间取消的晚到 DataSource dispose、connect 共享与 teardown、connecting/connected/completed 状态、operation prepare、Provider 注销后未派发 Operation 失效、execute availability 等待期间注销不派发、operation 单次执行共享、显式确认、execute 前 availability 复查、policyOverride 收紧、非空 plan unsupported、ExpressionBinding unsupported、legacy-command 映射。
...
- 直接调用 `sources/operations.register()` 的定义也会获得 core 内部 registration token，unregister/override/clear 会通知活动 Runtime；`override:true` 在新注册项注销后恢复被覆盖定义，但已被覆盖期间的旧 Runtime artifact 会被终止，避免继续使用已失效定义；
- `moduleRegistry.onChange()` 会在模块注册、资源替换、资源注销、模块卸载或清空时触发，core 订阅后自动执行 module Provider 对账；模块 Provider 消失会立即注销对应 token，替换 loader 会卸载旧 Provider 并按新 loader 重新加载，活动 Runtime artifact 按 Provider registration generation 精确失效。
