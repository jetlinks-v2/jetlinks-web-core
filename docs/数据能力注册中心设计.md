...
- `cloud.jetlinks.ui` 集成 PR 继续承载可视化 legacy binding adapter、子模块指针和宿主侧集成验证。core 侧测试覆盖 catalog、Provider 无参 single-flight 加载、Provider 并发注销 ghost 防护、手动 Provider 加载失败隔离、Provider 注册/卸载、直接 register 注销联动 Runtime、直接 register 多层 override 注册栈、active registration key 按 kind/id 隔离、Provider 多 definition 注册失败原子回滚、同 capabilityId 不同 Provider 代次精确清理、持久化 version mismatch、catalog availability 等待期间注销过滤、参数绑定、query 取消 reject 与 teardown、Provider 注销取消 pending availability/create/query 中的 snapshot query、connect 异步 create 期间取消的晚到 DataSource 单次 dispose、connect 共享与 teardown、connecting/connected/completed 状态、operation prepare、Provider 注销后未派发 Operation 失效、execute availability 等待期间注销不派发、operation 单次执行共享、显式确认、execute 前 availability 复查、policyOverride 收紧、非空 plan unsupported、ExpressionBinding unsupported、legacy-command 映射。
...
- 直接调用 `sources/operations.register()` 的定义也会获得 core 内部 registration token；底层 registry 按 capability id 保存完整注册栈，register/unregister/clear 统一计算 previousEffective → nextEffective，避免单个 unregister closure 私自恢复旧定义；direct definition 每次重新成为 effective 时生成新的 registrationId，active registration key 包含 capability kind 与 id；被覆盖或注销的有效定义会立即通知 Runtime，pending availability/create/query 中的 snapshot query、shared connection 和未派发 Operation 都会失效。
