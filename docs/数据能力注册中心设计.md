...
`registerProvider()` 只同步登记 Provider，definition 仍按需加载；Registry 内部 readiness 会先对账最新 module snapshot，再加载当前 pending Provider。`query()`、`connect()` 和 `prepareOperation()` 均在解析 capability ref 前等待同一个 single-flight readiness，因此持久化引用不依赖消费者先调用 `resolveCatalog()`。在缺少 Provider capability manifest 的当前契约下，readiness 只能加载全部 pending Provider，不伪造按 capability ID 精确路由。
...
| Runtime readiness | 已实现 | 持久化 ref 可直接 query/connect/prepare；并发加载 single-flight、Provider 失败隔离 | `ensureReady()`、core/module registry 测试 |
