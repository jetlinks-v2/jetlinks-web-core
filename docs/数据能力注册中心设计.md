...
- `tests/unit/dataCapabilityCore.spec.ts`、`tests/unit/dataCapabilityLegacy.spec.ts`：覆盖 catalog、provider 注册/卸载、多 Provider 同 scope 隔离、参数绑定、query、connect 共享与 teardown、operation prepare、operation 单次执行共享、execute 前 availability 复查、policyOverride 收紧、ExpressionBinding unsupported、legacy-command 映射与 legacy binding 转换。
