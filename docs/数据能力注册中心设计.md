...
| Registry、Runtime、Provider 生命周期 | `src/data-capability/registry.ts` |
...
| 行为测试 | `tests/unit/dataCapabilityCore.spec.ts`、`dataCapabilityLegacy.spec.ts`、`dataCapabilityModuleRegistry.spec.ts` |
...
这些能力可以统一归入“数据能力中心”，但不应全部放入 `DataCapabilityRegistry` 类。当前 Runtime 仍位于 `registry.ts`，后续实现高级能力前应先按上述职责拆分内部模块，并保持公共入口兼容。
...
- 完成、失败、取消、Provider 注销或 Runtime dispose 后释放资源。
...
| 能力 | 状态 | 当前结论 | 主要证据 |
| --- | --- | --- | --- |
| Provider 动态注册/注销 | 已实现 | 支持手动和 moduleRegistry Provider、对账和卸载 | `registry.ts`、module registry 测试 |
...
| RxJS query/connect | 已实现 | 支持首结果查询和持续连接 | Runtime、core 测试 |
...
| Operation prepare/confirm/execute | 已实现 | 快照、确认、单次派发和资源回收闭环 | Runtime、core/legacy 测试 |
| before-dispatch cancel | 已实现 | 仅派发前可取消，派发后拒绝伪取消 | Runtime、core 测试 |
| Operation 高级策略 | 未实现 | concurrency/idempotency/retry/audit/batch 无执行器 | Runtime、types |
...
目标：让已有公共契约形成自洽运行闭环，并为后续执行器建立边界。

1. 将 Runtime、source runner 和 operation runner 从超大的 `registry.ts` 拆到内部模块，公共导出保持兼容。
2. 增加 Runtime readiness：按持久化 capability ID 执行时自动确保已注册 Provider 完成加载，不要求先调用 catalog。
3. 建立 CapabilitySchema validator，在绑定解析后、Provider 调用前校验 config/query/input；preview 可选校验 output。
4. 为 output binding 定义中性的 Runtime 输出注入/更新入口，并检测悬空引用和循环依赖。
5. 为 OptionSource 提供统一执行服务，复用 binding、availability、AbortSignal、分页和错误模型。
6. 收窄 `policyOverride` 类型，禁止覆盖 Provider 的 concurrency 和 idempotency 事实。
7. 将三份 `dataCapability*.spec.ts` 接入一个稳定的 package test script，停止在长期文档中复制临时 bundle 命令。

验收：直接持久化引用可执行；无效 Schema 在 Provider 调用前失败；output/option 具备端到端测试；现有 Provider 生命周期测试全部通过。

### 9.2 P1：响应式参数与 MappingEngine

目标：支持组件选择时间、设备或其他上下文后，以友好 UI 生成并运行结构化配置。

1. BindingResolver 返回依赖集合，Runtime 建立 connection → parameter/context/output 依赖索引。
2. `updateParameters/updateContext/updateOutput` 只使受影响连接失效，并使用 switchLatest 取消旧连接。
3. 实现 MappingEngine v1：source/target schema 校验、字段建议、数组逐项映射、受控类型转换和 format。
4. preview 和 runtime 共用同一个 mapping executor，不允许配置预览与实际结果不一致。
...
1. 实现版本化 operator 白名单：`merge/combineLatest/switchLatest/forkJoin/bootstrapThenStream`。
2. 实现 `poll/retry/timeout`，明确整体超时、首结果超时和重试边界。
3. 实现 reconnect 状态机及最终失败条件。
4. 实现有界流控策略和 overflow 行为，禁止无界 ReplaySubject 或数组缓存。
5. 校验 plan DAG、节点版本、悬空输入和循环引用。
...
1. 为 resolved request 增加 Runtime 内部 requestId。
2. 实现有上限的 merge window、group、canMerge、merge 和 split。
3. 明确部分失败、缺失 split 结果、超时和消费者中途取消语义。
4. 对相同上游但不同 mapping 的连接评估“请求共享后分别投影”，在不改变结果的前提下提升共享率。
5. 增加批处理诊断：候选数、合并数、上游请求数、拆分失败和节省请求数。
...
以上 core 阶段不以可视化或智能体专用字段实现功能，但至少使用三个中性 fixture 验证：

- 一个 snapshot/page DataSource；
- 一个 stream DataSource，验证 teardown、重连和流控；
- 一个 high-risk Operation，验证 prepare、确认、幂等和审计边界。

完成 fixture 后再由可视化和智能体 adapter 分别验证目标 Schema、配置 UI 和 Tool 投影，不在 core 中增加消费者分支。
...
当前 capability 行为测试位于三份 `tests/unit/dataCapability*.spec.ts`，但尚未收敛成专用 package script。P0 应先建立稳定入口；脚本落地后，本文只保留脚本名称，不复制临时 esbuild/node 命令。

实现优化按阶段运行 capability 定向测试，并在阶段边界执行：
