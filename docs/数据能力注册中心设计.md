...
- `cloud.jetlinks.ui` 集成 PR 继续承载可视化 legacy binding adapter、子模块指针和宿主侧集成验证。core 侧测试覆盖 catalog、Provider 无参 single-flight 加载、Provider 并发注销 ghost 防护、Provider 注册/卸载、同 capabilityId 不同 Provider 代次精确清理、参数绑定、query、connect 共享与 teardown、connecting/connected/completed 状态、operation prepare、Provider 注销后未派发 Operation 失效、operation 单次执行共享、显式确认、execute 前 availability 复查、policyOverride 收紧、非空 plan unsupported、ExpressionBinding unsupported、legacy-command 映射。
