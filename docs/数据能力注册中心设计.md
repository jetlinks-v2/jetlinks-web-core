...
- Provider 懒加载和模块生命周期；
- 定义版本、来源模块和变化通知；
- 按 Provider 声明的中性元数据进行索引和过滤。

Registry 不负责根据租户、用户、项目、菜单、路由或消费者类型推断能力集合。当前前端运行时能看到什么能力，首先由宿主模块注册了哪些 Provider 决定；Provider 是能力定义、分组、分类、排序、Schema、UI metadata 和运行策略的事实来源。
...
Runtime 以项目、页面、预览实例或其它宿主会话作为生命周期作用域。core 不直接认识可视化 `ComponentSchema`，组件由可视化适配层转换成带稳定 `consumerId` 的连接请求。Runtime context 只被透传给 Provider、availability 和 BindingResolver，core 不基于其中字段推断业务隔离。
...
- Provider 应提供所有目录展示元数据，包括分组、分类、标签、排序和 UI schema；core 只存储、匹配和透传，不解释业务含义。
...
Provider 注册不依赖当前菜单是否已经加载。宿主模块没有对应功能、未加载或被卸载时，应不注册或主动注销对应 Provider；Provider 已注册但当前不可发现、不可配置或不可执行时，通过 definition 的 `availability(context, phase)` 表达。菜单、按钮权限和后端状态可以作为 Provider 自己判断 availability 的输入，但 core 不内置菜单、租户、用户或项目语义。
...
Provider 默认返回冷 Observable：订阅时才建立请求或后端订阅，unsubscribe 时必须执行 teardown。即使底层 WebSocket 是热流，Provider 也要将监听注册和注销包装成完整 Observable 生命周期。Provider 不自行永久 `shareReplay`；是否共享由 Runtime 根据完整标准化执行 key 和 Provider 策略决定。
...
- Provider 标准化后的配置和查询；
- 执行模式、轮询和流控策略；
- Provider 标准化后显式写入请求的流控、分页和执行约束；
- 影响请求结果的中性参数和配置。
...
Provider 由拥有能力的宿主模块注册。没有对应模块、功能或组件时，不应把能力注册到 Registry；已经注册的能力是否当前可发现、可配置或可执行，由 Provider 的 availability 返回。

`discoverable`、`configurable`、`executable` 分别约束目录展示、配置保存和真实执行；菜单可见性最多作为 Provider 自己判断 discoverable 的输入线索，不能代表执行授权。
...
1. 实现通用 registry kernel、变化通知和 Provider loader；Provider definitions 必须由 Provider/宿主注册事实决定，core 不根据租户、用户、项目或隔离上下文推断能力集合。
...
1. 公共上下文只保留 `parameters` 与 `attributes`，`RuntimeCreateContext` 只额外声明 `runtimeId`；不暴露 `scopeId`、`isolationKey`、租户、用户、项目、菜单或路由字段。
2. `DataCapabilityProvider.load()` 改为无参且每个 Provider token 只加载一次；当前能力集合由宿主注册或注销 Provider 决定，不通过上下文差异重复加载。
3. Provider 注销时移除其注册的 definitions，并通知活动 Runtime 终止相关 DataSource 连接，发出 `unavailable` 状态、取消上游订阅、触发 `AbortSignal` 并调用 `dispose()`。
4. Runtime 所有入口在 `dispose()` 后抛出 `runtime.disposed`；dispose 统一清理连接、操作订阅、prepared/confirmed/execution 缓存和 Provider 实例资源。
5. Operation 执行必须经过显式确认协议：无需确认的操作可直接使用准备快照，要求确认的操作必须通过 `confirmOperation()` 取得 confirmed handle 后才能执行。
6. DataConnection 分离生命周期状态与最新数据回放，有限数据源即使已发出数据也必须继续发出 `completed` 状态。
7. 当前 MVP 不支持非空 `binding.plan`；遇到 plan 节点时以 `data_source.plan.unsupported` 明确失败，避免静默忽略组合查询语义。


本轮实现验证：

- `dataCapabilityCore.spec.ts` 覆盖 Provider 无参单次加载、Provider 注销终止 DataSource、运行时 dispose 守卫、操作确认、有限数据源 completed、latest replay 和 `binding.plan` unsupported。
- `dataCapabilityLegacy.spec.ts` 覆盖 legacy-command Provider 在无业务上下文 `load()` 下仍可生成数据源和操作适配能力。
- 已执行 core 单测 bundle、`diff --check`、`visualization-manager-ui` 指定模块构建和 core 默认构建；`vue-tsc` 当前仓库存在既有非 data-capability 类型错误，本轮相关路径未出现新增错误。
...
- 宿主注册或注销 Provider 后，catalog 能通过变化通知重新计算。
...
- 完全相同请求按 executionKey 共享，Provider 标准化请求或执行策略不同时不共享。
...
- 宿主未注册的能力不会出现在目录中；直接构造引用仍会因 capability 未注册或后端权限校验被拒绝。
- 请求共享只发生在完整标准化 executionKey 相同的连接之间，不根据业务上下文做 core 内置分区。
...
5. Provider 注册独立于菜单加载；宿主没有对应功能时不注册 Provider，已注册能力的 discover/configure/execute 状态由 Provider availability 表达。
...
`CapabilityContext` 是传给 Provider、availability 和 BindingResolver 的中性运行时线索。core 不解释这些线索，不根据租户、用户、项目、菜单、路由或消费者类型推断能力集合、分组或缓存隔离。
...
- `parameters` 承载运行时参数，例如页面、设计器或智能体运行时传入的可绑定值；
- `attributes` 承载宿主自定义上下文，core 只透传给 Provider，不读取其中字段；
- core 不提供 `tenantId`、`userId`、`projectId`、`pageId`、`routeName`、`menuCode`、`isolationKey` 等业务隔离字段作为公共契约；如宿主确实需要这些信息，应放入 `attributes` 并由自己的 Provider 解释；
- 用户没有对应模块、组件或功能时，宿主应不注册对应 Provider，而不是要求 core 根据上下文过滤；
- Provider 已注册但当前不可用时，用 `availability(context, phase)` 返回不可发现、不可配置或不可执行。

该设计保持 core 的职责为“当前前端运行时能力注册表 + 运行时协议”，而不是“业务权限/租户/页面上下文裁剪器”。
...
`tags` 和 `facets` 是 Provider 提供的中性筛选元数据。例如 Provider 可以声明 `domain=device`、`realtime=true`、`risk=read`、`group=telemetry`，core 只做匹配、排序和透传，不解释业务含义。目录分组、分类、图标、排序等展示信息也应由 Provider 通过标准字段或 `facets` 提供，消费者负责选择如何展示。
...
Provider 是能力事实来源，负责提供 definitions 以及所有目录元数据：名称、描述、分组、分类、标签、排序、Schema、UI metadata、运行策略和 availability。`load()` 不接收业务上下文，返回该 Provider 拥有的稳定能力定义。

不允许在 Provider `load()` 中按租户、用户、项目、页面或菜单生成不同 definitions；这会让 Registry 变成业务上下文缓存。动态能力集合应通过宿主注册/注销不同 Provider 实例表达，动态可见性和执行性应通过 `availability(context, phase)` 表达。命令目录、REST API、WebSocket / Topic、本地计算或第三方能力都先由 Provider 转换为中性 definitions，再交给 core 注册。
...
命令可以动态注册为数据能力，但命令不是 core 的一等概念。`legacy-command` Provider 负责读取命令目录，并按命令语义生成中性的 DataSource 或 Operation。命令目录需要因宿主功能或业务入口变化而改变集合时，由宿主重新注册/注销对应 Provider；如果只影响可见或可执行状态，则由 Provider 的 availability 表达。
...
- pageType、工程或宿主入口变化如果会改变命令集合，应由宿主重新注册/注销 legacy Provider；如果只影响可见或可执行状态，应由 Provider availability 表达。
...
5. `DataCapabilityRuntime` 的 `query`、`connect`、`preview`、`prepareOperation`、`confirmOperation`、`executeOperation` 最小闭环；
6. HTTP AbortSignal、Observable teardown、Provider 注销联动、运行时 dispose 和非空 plan unsupported 保护；
7. legacy-command provider skeleton；
8. 数据能力测试页路由，用于目录查看、配置预览、查询验证和操作 dry-run / 低风险 execute 验证；
...
- 跨 Provider 编排图和 `binding.plan` runner；
- poll timer、retry、reconnect 和持久缓存策略；
- Operation concurrency 队列、keyed idempotency 自动生成、cancel/compensation 执行器；
- 长任务恢复；
- 完整低代码表单渲染器；
- 全量旧项目持久化迁移。
...
- 不配置 `skipMenuFetch`，避免诊断入口绕过菜单可见性检查；
...
│ parameters attributes keyword kind provider tag facet...      │
├───────────────┬───────────────────────────────┬──────────────┤
...
- `src/types/module.ts`、`src/utils/module-registry.ts`：`moduleRegistry` 资源类型新增 `dataCapabilityProviders`，并提供 `onChange()` 订阅用于模块 Provider 实时对账；
...
- `tests/unit/dataCapabilityCore.spec.ts`、`tests/unit/dataCapabilityLegacy.spec.ts`、`tests/unit/dataCapabilityModuleRegistry.spec.ts` 已进入 core PR，作为 Registry、Runtime、legacy-command provider 和动态模块 Provider 生命周期的可复核最小运行测试；
- `cloud.jetlinks.ui` 集成 PR 继续承载可视化 legacy binding adapter、子模块指针和宿主侧集成验证。core 侧测试覆盖 catalog、Provider 无参 single-flight 加载、Provider 并发注销 ghost 防护、手动 Provider 加载失败隔离、Provider 注册/卸载、直接 register 注销联动 Runtime、直接 register 多层 override 注册栈、active registration key 按 kind/id 隔离、Provider 多 definition 注册失败原子回滚、同 capabilityId 不同 Provider 代次精确清理、持久化 version mismatch、catalog availability 等待期间注销过滤、参数绑定、query 取消 reject 与 teardown、Provider 注销取消 pending availability/create/query 中的 snapshot query、query 取消后晚到 DataSource 自动 dispose、connect 异步 create 期间取消的晚到 DataSource 单次 dispose、connect 共享与 teardown、connecting/connected/completed 状态、operation prepare、Provider 注销后未派发 Operation 失效、execute availability 等待期间注销不派发、operation 单次执行共享、显式确认、Provider PreparedOperation 凭证透传、unsupported / legacy Operation 不暴露取消、before-dispatch 取消、派发后不伪造撤销、执行中 prepared 不受 TTL/LRU 回收、prepared 缓存有界清理、execute 前 availability 复查、policyOverride 收紧、moduleRegistry 并发 catalog single-flight、加载中 loader 替换、资源注销和模块卸载、非空 plan unsupported、ExpressionBinding unsupported、legacy-command 映射。

本轮验证结果：

```bash
pnpm -C ui exec esbuild jetlinks-web-core/tests/unit/dataCapabilityCore.spec.ts --bundle --platform=node --format=esm --target=node22 --outfile=/tmp/dataCapabilityCore.mjs --external:vue --external:ant-design-vue --external:@jetlinks-web/vite --external:@jetlinks-web-core/utils/consts --external:@jetlinks-web-core/utils/module-registry && node /tmp/dataCapabilityCore.mjs

pnpm -C ui exec esbuild jetlinks-web-core/tests/unit/dataCapabilityLegacy.spec.ts --bundle --platform=node --format=esm --target=node22 --outfile=/tmp/dataCapabilityLegacy.mjs --external:vue --external:ant-design-vue --external:@jetlinks-web/vite --external:@jetlinks-web-core/utils/consts --external:@jetlinks-web-core/utils/module-registry && node /tmp/dataCapabilityLegacy.mjs

pnpm -C ui exec esbuild jetlinks-web-core/tests/unit/dataCapabilityModuleRegistry.spec.ts --bundle --platform=node --format=esm --target=node22 --outfile=/tmp/dataCapabilityModuleRegistry.mjs --external:vue --external:ant-design-vue --define:import.meta.env='{}' && node /tmp/dataCapabilityModuleRegistry.mjs
...
- `/data-capability/lab` 默认不会进入生产核心路由；仅开发环境或显式 `VITE_DATA_CAPABILITY_LAB=true` 时可作为人工验收入口。选择带 `ui.preview`、表单 `editor` 或 `value-editor.editor` 的能力时，会加载注册组件并注入当前 capability、context、草稿 config/query/input、运行 result/events；测试页事件面板只保留最近 200 条事件并展示已裁剪数量，避免无界流导致页面内存无限增长；
...
- Provider 是能力事实来源，负责提供 definitions、分组、分类、排序、Schema、UI metadata、policy 和 availability；core 不把业务上下文作为公共缓存隔离语义，宿主没有对应功能时应不注册对应 Provider；

- Provider 注册改为每次注册生成唯一 token，注册项、scope 和注销函数按 token 精确绑定，避免同 scope 多 Provider 或同 Provider 多 scope 相互覆盖；
- Runtime connect 会按标准化请求和 Provider 注册代次生成 executionKey，共享完全相同请求，并在最后一个消费者取消时取消上游 subscription、abort signal 并 dispose DataSource；
- Operation execute 会在 Runtime 内部只订阅一次 Provider Observable，并用 ReplaySubject 共享同一次执行事件，避免多观察者重复触发副作用；
- Runtime 对外返回不可冲突的 PreparedOperation runtime id，但 Provider execute 收到 prepare 阶段原始 PreparedOperation，保留后端 dry-run token、预检单号等 Provider 私有凭证；执行结束、取消、Provider 注销或 prepared 缓存超过 TTL/数量上限时，会释放 Operation、confirmed handle、execution 和事件缓冲。
...
- 当前 core MVP 暂不执行 poll/retry/reconnect/cache、Operation concurrency/keyed idempotency/cancel 等高级 Runner 策略；这些字段作为 Provider policy 和后续执行器契约保留，不能在当前 PR 中宣称为已生效能力；
- Provider 加载使用 token + generation 的 single-flight 状态；并发 resolve 共享同一个 load promise，注销后旧 promise 结果会被丢弃，不再注册 ghost definitions；
- Runtime artifact 记录 Provider 注册代次，Provider 注销只清理精确匹配的 DataSource 和未派发 Operation，不误杀后续覆盖同 capabilityId 的 Provider 实例；

- 单次 `query()` 纳入显式取消语义：外部 `AbortSignal`、Runtime dispose 或内部取消都会 reject 调用方 Promise，并执行 unsubscribe、abort 和 DataSource dispose；
- 执行前会校验持久化 capability version，版本不一致返回 `capability.version_mismatch`，后续 migration runner 落地后再开放自动迁移；
- 直接调用 `sources/operations.register()` 的定义也会获得 core 内部 registration token；底层 registry 按 capability id 保存完整注册栈，register/unregister/clear 统一计算 previousEffective → nextEffective，避免单个 unregister closure 私自恢复旧定义；definition 每次重新成为 effective mount 时生成新的 registrationId；Provider ownership identity 只表示来源 token/generation，effective mount identity 才参与 Runtime artifact 校验；active registration key 包含 capability kind 与 id；被覆盖或注销的有效定义会立即通知 Runtime，pending availability/create/query 中的 snapshot query、shared connection 和未派发 Operation 都会失效，已标记 cancelled/disposed 的 execution 即使旧异步 availability 后续返回也不会派发。
- Provider 多 definition 注册按批次原子处理：任一 definition 注册失败时会反向执行已注册项 unregister、清理 definition owner 和 providerDefinitionIds，避免目录半注册。
- `moduleRegistry` 首次加载与 `onChange()` 刷新共用同一条串行对账队列；模块注册、资源替换、资源注销、模块卸载或清空都会触发 module Provider 对账。模块 Provider 消失会立即注销对应 token，替换 loader 会卸载旧 Provider 并按新 loader 重新加载，module token 的 generation 单调递增且每次有效注册拥有不可复用 registrationId，活动 Runtime artifact 按 registrationId 精确失效；刷新期间再次收到变化会标记 dirty 并追加第二轮对账，加载中的旧快照若变脏会被丢弃。
- `resolveCatalog()` 在 definition availability await 后会重新校验 registration 是否仍有效，防止旧目录请求晚返回已注销或已被覆盖的 definition。

本轮评审修复结果：

1. Operation 显式取消契约已按可兑现能力收敛：`cancellation: 'unsupported'` 不暴露 `cancel()`；`before-dispatch` 只在真正派发前提供取消入口并产生 `cancelled` 事件，派发后删除入口，已捕获的旧函数也会明确拒绝，不会 unsubscribe 已派发副作用的结果流。Provider 尚未提供真实取消契约时，`best-effort` / `compensatable` 同样不暴露不可兑现的入口；legacy command 保持 `unsupported`。
2. prepared TTL/LRU 只回收未执行条目；已经进入 `operationExecutions` 的 Operation 由完成、失败、真实取消、Provider 注销或 Runtime dispose 负责释放，长任务不会因后续 `prepareOperation()` 触发清理而被中途 dispose。
3. module Provider 的“确保已加载”和“模块确有变化”已分离：并发 `resolveCatalog()` 只复用当前串行对账 Promise，不设置 dirty；只有 `moduleRegistry.onChange()` 才追加脏快照对账。
4. 行为测试已覆盖 legacy/unsupported 不暴露取消、before-dispatch 取消、派发后不因伪取消丢失结果、执行中 prepared 不被 TTL/LRU 淘汰、并发 catalog 不重载慢 Provider，以及 moduleRegistry 注册、加载中 loader 替换、资源注销和模块卸载的动态路径。
5. 本轮不扩展 Operation 并发调度、补偿操作实现或业务命令撤回协议；这些能力仍需由后续中性 Provider 取消契约和 Runner 策略实现。

主要风险是取消契约属于公共 API：实现必须保持 `cancel` 可选，并以“能力可兑现”而非仅凭 policy 字段决定是否暴露，避免消费者把本地停止监听误判为副作用撤销。
