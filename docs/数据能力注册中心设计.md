...
- `cloud.jetlinks.ui` 集成 PR 继续承载可视化 legacy binding adapter、子模块指针和宿主侧集成验证。core 侧测试覆盖 catalog、Provider 无参 single-flight 加载、Provider 并发注销 ghost 防护、手动 Provider 加载失败隔离、Provider 注册/卸载、直接 register 注销联动 Runtime、直接 register 多层 override 注册栈、active registration key 按 kind/id 隔离、Provider 多 definition 注册失败原子回滚、同 capabilityId 不同 Provider 代次精确清理、持久化 version mismatch、catalog availability 等待期间注销过滤、参数绑定、query 取消 reject 与 teardown、Provider 注销取消 pending availability/create/query 中的 snapshot query、query 取消后晚到 DataSource 自动 dispose、connect 异步 create 期间取消的晚到 DataSource 单次 dispose、connect 共享与 teardown、connecting/connected/completed 状态、operation prepare、Provider 注销后未派发 Operation 失效、execute availability 等待期间注销不派发、operation 单次执行共享、显式确认、execute 前 availability 复查、policyOverride 收紧、非空 plan unsupported、ExpressionBinding unsupported、legacy-command 映射。
...
- `moduleRegistry.onChange()` 会在模块注册、资源替换、资源注销、模块卸载或清空时触发，core 订阅后自动执行 module Provider 对账；模块 Provider 消失会立即注销对应 token，替换 loader 会卸载旧 Provider 并按新 loader 重新加载，module token 的 generation 单调递增且每次有效注册拥有不可复用 registrationId，活动 Runtime artifact 按 registrationId 精确失效；刷新期间再次收到变化会标记 dirty 并追加第二轮对账。
