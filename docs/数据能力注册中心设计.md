...
- `high` / `critical` 操作默认禁用真实执行，除非当前环境和权限都允许，并要求用户输入 capability id 二次确认；
...
- `src/router/basic.ts`、`src/views/data-capability/lab/`：隐藏路由 `/data-capability/lab`，用于目录查看、注册 UI 组件预览、JSON 草稿配置、DataSource preview/query/connect、OptionSource query、Operation prepare/execute 验证；
- `modules/visualization-manager-ui/dataCapabilities/legacyCommandProvider.ts`：可视化旧命令注册为中性数据能力的 provider；
- `modules/visualization-manager-ui/components/Designer/dataSource/legacyCapabilityAdapter.ts`：旧 `dataSourceProps` / `actionData` 到中性 binding 的转换入口；
- `tests/unit/dataCapabilityCore.spec.ts`、`tests/unit/dataCapabilityLegacy.spec.ts`：覆盖 catalog、provider 注册/卸载、参数绑定、query、operation prepare、legacy-command 映射与 legacy binding 转换。
...
- core MVP 与可视化 legacy provider 已可构建；
- `/data-capability/lab` 已被生产构建打包，可作为人工验收入口；选择带 `ui.preview`、表单 `editor` 或 `value-editor.editor` 的能力时，会加载注册组件并注入当前 capability、context、草稿 config/query/input、运行 result/events；
