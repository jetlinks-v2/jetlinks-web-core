...
- `cloud.jetlinks.ui` 集成 PR 继续承载可视化 legacy binding adapter、子模块指针和宿主侧集成验证。core 侧测试覆盖 catalog、provider 注册/卸载、参数绑定、query、connect 共享与 teardown、operation prepare、operation 单次执行共享、execute 前 availability 复查、policyOverride 收紧、ExpressionBinding unsupported、legacy-command 映射。
